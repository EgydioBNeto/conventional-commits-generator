...
def delete_commit(commit_hash: str) -> bool:
    """Delete a specific commit from the git history.

    This works for any commit including initial commits and latest commits.

    Args:
        commit_hash: Full or short hash of the commit to delete

    Returns:
        bool: True if successful, False otherwise
    """
    print_process(f"Preparing to delete commit {commit_hash[:8]}...")

    # Check if this is the current HEAD commit
    success, head_hash = run_git_command(
        ["git", "rev-parse", "HEAD"],
        "Failed to get current HEAD commit hash",
        show_output=True
    )
    ...
    # Check if this is the only commit in the repository
    ...
    is_only_commit = commit_count.strip() == "1"
    is_head_commit = commit_hash.startswith(head_hash) or head_hash.startswith(commit_hash)

    # Case 1: It's the only commit in the repository
    if is_only_commit:
        print_warning("This is the only commit in the repository.")
        print_warning("Deleting it will reset the repository to its initial state.")

        success, _ = run_git_command(
            ["git", "update-ref", "-d", "HEAD"],
            "Failed to delete the initial commit",
            "Repository has been reset to its initial state"
        )
        return success

    # Case 2: It's the HEAD commit (most recent commit)
    elif is_head_commit:
        print_info("Deleting the most recent commit (HEAD)...")

        # Use reset --hard to remove the commit
        success, _ = run_git_command(
            ["git", "reset", "--hard", "HEAD~1"],
            "Failed to delete the HEAD commit",
            "Most recent commit has been deleted"
        )
        return success

    # Case 3: It's an initial commit (root commit)
    else:
        # Check if this is a root commit
        success, root_commits = run_git_command(
            ["git", "rev-list", "--max-parents=0", "HEAD"],
            "Failed to identify root commits",
            show_output=True
        )
        ...
        is_root_commit = commit_hash in root_commits

        if is_root_commit:
            print_info("Deleting a root commit...")
            ...
            # Use filter-branch to remove the root commit
            # This creates an orphaned history from the commit onwards
            success, _ = run_git_command(
                ["git", "filter-branch", "--parent-filter",
                 f"sed 's/^-p {commit_hash}//'", "--force", "HEAD"],
                "Failed to rewrite history to remove the root commit",
                "Root commit has been deleted from history"
            )
            ...
            run_git_command(
                ...
                "Warning: Failed to run garbage collection",
                "Repository has been optimized"
            )

            return True

        # Case 4: It's a regular commit in the middle of history
        else:
            print_info("Deleting a commit from the middle of history...")

            # Use rebase --onto to skip the commit
            # First, get the parent commit
            success, parent_hash = run_git_command(
                ["git", "rev-parse", f"{commit_hash}^"],
                "Failed to get parent commit hash",
                show_output=True
            )
            ...
            # Now rebase to skip the commit
            print_process(f"Rebasing to skip commit {commit_hash[:8]}...")
            success, _ = run_git_command(
                ["git", "rebase", "--onto", parent_hash, commit_hash],
                "Failed to rebase and remove the commit",
                "Commit has been removed from history"
            )
